...
export default function CommunityAds({ projectAds }) {
   const [handle, setHandle] = useState(true)
   return <>
      {handle && <figure className="relative flex flex-col lg:flex-row my-8 group rounded-xl border">
         <button onClick={() => setHandle(!handle)} className="absolute top-3 right-3 bg-white flex justify-center size-9 z-20 hover:bg-gray-200 rounded-full items-center text-xl"><FontAwesomeIcon icon={faXmark} /></button>
         {/* Community Image */}
         <div className="overflow-hidden flex-1 max-sm:rounded-t-xl">
         <Image src={projectAds.offerimage} width={850} height={550} alt={projectAds.h1} title={projectAds.h1} className=" w-full group-hover:scale-110 transition-all min-h-full" />
         </div>
         {/* Community Information */}
         <figcaption className="bg-white lg:p-8 p-6  flex-1">
            <h2 className="text-xl md:text-2xl mb-2">{projectAds.offerheading}</h2>
            <div className="mb-4 text-zinc-600 text-sm"><FontAwesomeIcon icon={faLocationDot} /> Location Name</div>
            ...
            <div className="flex flex-wrap items-center gap-3">
               ...
               <WhatsappLink phoneNumber={projectAds.WhatsApp} proName={projectAds.offerheading} className="flex items-center relative z-10">
                  <FontAwesomeIcon icon={faSquareWhatsapp} className="text-[var(--whatsapp)] size-[50px]" />
               </WhatsappLink>
