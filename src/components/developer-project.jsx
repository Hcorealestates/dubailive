...
export default function DeveloperProject({ devProject, className }) {
  return <figure className={`${className} relative bg-white rounded-lg *:rounded-lg group`}>
    <Image src={devProject.listingimage} width={500} height={400} className="w-screen group-hover:brightness-50 group-hover:drop-shadow-xl transition-all h-80" alt={devProject.propname} title={devProject.propname} />
    <Link href={devProject.propurl}>
      <span className="absolute inset-0 z-1"></span>
    </Link>
    <figcaption className="absolute bottom-3 left-3 mr-3 bg-white p-4">
      <h4 className="text-xl md:text-2xl mb-2 line-clamp-2 group-hover:text-primary group-hover:translate-x-2 transition-transform">{devProject.propname}</h4>
      <Link className="link text-sm mb-2 inline-block relative z-10 hover:translate-x-3 transition-transform" href={devProject.proplocationurl}><FontAwesomeIcon icon={faLocationDot} />  {devProject.proplocation}
      </Link>
      {devProject.propsize &&
